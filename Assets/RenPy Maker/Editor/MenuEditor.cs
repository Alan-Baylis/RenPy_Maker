    ...
    public override void OnBodyGUI()
    {
        ...
        if (EditorGUI.EndChangeCheck())
        {
            if (_menuNode.GetCharacterIndex() < set.Count)
            {
                ...
                {
                    if (n.GetNodeType() == "CharacterNode" && set[_menuNode.GetCharacterIndex()] == n.GetCharacter())
                    {
            ...
            switch (_currentNodeTab)
            {
                case 0:
                    ...
                    if (GUILayout.Button("Create New Button"))
                    {
                        ...
                        _menuNode.AddDynamicOutput(typeof(int), Node.ConnectionType.Override, Node.TypeConstraint.None, _newMenuOptionOutput);
