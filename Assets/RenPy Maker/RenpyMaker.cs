	...
	void MakeRenpyScript()
	{
		...
		foreach (string item in directories)
		{
			if (Path.GetFileName(item) == _nodeParser.projectName)
			{
				...
				{
					NodeGraph tempGraph = Resources.Load(_nodeParser.projectName + "/" + Path.GetFileNameWithoutExtension(files[i].Name)) as NodeGraph;
					if (tempGraph != null)
					{
			...
			switch (node.GetNodeType())
			{
				...
				case "MenuNode":
				{
					...
					for (int i = 0; i < optionList.Count(); i++)
					{
						...
						WriteString(_fullPath, "        jump " + optionList[i].option + "_" + node.GetNodeId(), true);
						WriteString(_fullPath, "", true);
					}
